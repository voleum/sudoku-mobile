...
export class AchievementEvaluator {
  ...
  evaluateGameCompletion(
    ...
    // Skill achievements (difficulty mastery)
    if (context.difficulty) {
      rules.push(...this.evaluateDifficultyMasteryAchievements(context, currentAchievements));
    }

    // Persistence achievements (streaks)
    if (context.currentStreak !== undefined) {
      rules.push(...this.evaluateStreakAchievements(context, currentAchievements));
    }

    // Exploration achievements
    rules.push(...this.evaluateExplorationAchievements(context, currentAchievements));
  ...
  /**
   * Difficulty mastery achievements: Expert for each difficulty level
   */
  private evaluateDifficultyMasteryAchievements(
    context: AchievementEvaluationContext,
    currentAchievements: Achievement[]
  ): EvaluationRule[] {
    const rules: EvaluationRule[] = [];

    // Map difficulty levels to achievement IDs
    const difficultyMasteryMap: Record<DifficultyLevel, AchievementId> = {
      'beginner': AchievementId.BEGINNER_EXPERT,
      'easy': AchievementId.EASY_MASTER,
      'medium': AchievementId.MEDIUM_MASTER,
      'hard': AchievementId.HARD_MASTER,
      'expert': AchievementId.EXPERT_MASTER
    };

    const achievementId = context.difficulty ? difficultyMasteryMap[context.difficulty] : undefined;
    if (achievementId) {
      const achievement = currentAchievements.find(a => a.id === achievementId);
      if (achievement && !achievement.isUnlocked) {
        const currentProgress = achievement.progressCurrent + 1;
        rules.push({
          achievementId,
          shouldUpdate: true,
          newProgress: currentProgress,
          shouldUnlock: currentProgress >= achievement.progressTarget
        });
      }
    }
  ...
  /**
   * Streak achievements: 3, 5, 10, 30 consecutive wins
   */
  private evaluateStreakAchievements(
    context: AchievementEvaluationContext,
    currentAchievements: Achievement[]
  ): EvaluationRule[] {
    const rules: EvaluationRule[] = [];
    ...
    const streakAchievements = [
      { id: AchievementId.STREAK_3, target: 3 },
      { id: AchievementId.STREAK_5, target: 5 },
      { id: AchievementId.STREAK_10, target: 10 },
      { id: AchievementId.STREAK_30, target: 30 }
    ];

    for (const streak of streakAchievements) {
      const achievement = currentAchievements.find(a => a.id === streak.id);
      if (achievement && !achievement.isUnlocked && currentStreak >= streak.target) {
        rules.push({
          achievementId: streak.id,
          shouldUpdate: true,
  ...
  /**
   * Exploration achievements: Explorer(try all levels), Versatile(complete all levels)
   */
  private evaluateExplorationAchievements(
    context: AchievementEvaluationContext,
    currentAchievements: Achievement[]
  ): EvaluationRule[] {
    const rules: EvaluationRule[] = [];

    // Note: These achievements need to track across multiple games
    // The actual tracking is done in the repository/use case layer
    // This method is here for completeness but may not be called directly from game completion

    return rules;
  }

  /**
   * Check if any error-free streak achievements should be updated
   */
  evaluateErrorFreeStreak(
    consecutiveErrorFreeGames: number,
    currentAchievements: Achievement[]
  ): EvaluationRule[] {
    const rules: EvaluationRule[] = [];

    const errorFreeStreak = currentAchievements.find(a => a.id === AchievementId.ERROR_FREE_STREAK);
    if (errorFreeStreak && !errorFreeStreak.isUnlocked) {
      rules.push({
        achievementId: AchievementId.ERROR_FREE_STREAK,
        shouldUpdate: true,
        newProgress: consecutiveErrorFreeGames,
        shouldUnlock: consecutiveErrorFreeGames >= errorFreeStreak.progressTarget
      });
    }
  ...
  /**
   * Check if any hint-free streak achievements should be updated
   */
  evaluateHintFreeStreak(
    consecutiveHintFreeGames: number,
    currentAchievements: Achievement[]
  ): EvaluationRule[] {
    const rules: EvaluationRule[] = [];

    const hintFreeStreak = currentAchievements.find(a => a.id === AchievementId.HINT_FREE_STREAK);
    if (hintFreeStreak && !hintFreeStreak.isUnlocked) {
      rules.push({
        achievementId: AchievementId.HINT_FREE_STREAK,
        shouldUpdate: true,
        newProgress: consecutiveHintFreeGames,
        shouldUnlock: consecutiveHintFreeGames >= hintFreeStreak.progressTarget
      });
    }
